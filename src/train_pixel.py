...
def train(epoch, state_dict, model, optimizer, train_loader, valid_loader, args, logger):
    ...
    for data in tqdm(train_loader):
        ...
        if not state_dict['itr'] % args.save_every:
            ...
            save_image(y.cpu(), args.output_dir + 'train_low_img_128_itr{}.png'.format(state_dict['itr']))
            save_image(x_hat.cpu(), args.output_dir + 'train_recon_img_128_itr{}.png'.format(state_dict['itr']))
            save_image(x.cpu(), args.output_dir + 'train_real_img_128_itr{}.png'.format(state_dict['itr']))

            # Save model & optimizer weights
            torch.save({
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                }, args.output_dir + '/UNET_pixel_model_128_itr{}.pth'.format(state_dict['itr']))
