...
if __name__ == "__main__":
    ...
    # Create dataset
    dataset_dir = os.path.join(args.root_dir, 'data/celeba_512/celebaHQ512')
    ...
    wt_model = WT(wt=wt, num_wt=args.num_iwt + 2)
    ...
    img_output_dir = os.path.join(args.root_dir, 'wtvae_results/image_samples/iwtae128_{}'.format(args.config))
    model_dir = os.path.join(args.root_dir, 'wtvae_results/models/iwtae128_{}/'.format(args.config))
    log_dir = os.path.join(args.root_dir, 'runs/iwtae128_{}'.format(args.config))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_iwtae_iwtmask128(epoch, wt_model, iwt_model, optimizer, iwt_fn, train_loader, train_losses, args, writer)
        eval_iwtae_iwtmask128(epoch, wt_model, iwt_model, optimizer, iwt_fn, sample_loader, args, img_output_dir, model_dir, writer, not args.nosave)
