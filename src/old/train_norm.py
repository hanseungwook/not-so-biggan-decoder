...
for epoch in range(0, 500):
    ...
    for i, data in enumerate(dataloader_vanilla, 0):
        ...
        real_mask_tr_norm = normalize(real_mask_tr, shift_64, scale_64)
        real_mask_bl_norm = normalize(real_mask_bl, shift_64, scale_64)
        real_mask_br_norm = normalize(real_mask_br, shift_64, scale_64)
        
        # Normalize 128 level masks and divide into 32 x 32 patches
        real_mask_tr_patches = create_patches_from_grid(real_mask_tr_norm)
        real_mask_bl_patches = create_patches_from_grid(real_mask_bl_norm)
        real_mask_br_patches = create_patches_from_grid(real_mask_br_norm)
        ...
        with torch.no_grad():
            ...
            # Denormalize masks from first level
            masks_tr_32 = denormalize(mask_tr_32, shift_32, scale_32)
            masks_bl_32 = denormalize(mask_bl_32, shift_32, scale_32)
            masks_br_32 = denormalize(mask_br_32, shift_32, scale_32)
    ...
    recon_mask_tr_denorm = denormalize(recon_mask_tr, shift_64, scale_64)
    recon_mask_bl_denorm = denormalize(recon_mask_bl, shift_64, scale_64)
    recon_mask_br_denorm = denormalize(recon_mask_br, shift_64, scale_64)
    
    recon_mask_tr_img = collate_channels_to_img(recon_mask_tr_denorm, DEVICE0)
    recon_mask_bl_img = collate_channels_to_img(recon_mask_bl_denorm, DEVICE0)   
    recon_mask_br_img = collate_channels_to_img(recon_mask_br_denorm, DEVICE0)
    ...
    Y_32_low = iwt(Y_real, inv_filters, levels=3)
    ...
    with torch.no_grad():
        ...
        pg_mask_tr_32 = denormalize(pg_mask_tr_32, shift_32, scale_32)
        pg_mask_bl_32 = denormalize(pg_mask_bl_32, shift_32, scale_32)
        pg_mask_br_32 = denormalize(pg_mask_br_32, shift_32, scale_32)
        ...
        # Denormalize 2nd level masks
        pg_mask_tr_64_img = denormalize(pg_mask_tr_64_img, shift_64, scale_64)
        pg_mask_bl_64_img = denormalize(pg_mask_bl_64_img, shift_64, scale_64)
        pg_mask_br_64_img = denormalize(pg_mask_br_64_img, shift_64, scale_64)     
